...
اكتب له محتوى صفحة متجره بالعربية الخليجية الواضحة:
...
الصورة ممكن تكون غلاف، تصميم، إعلان، بوستر، لقطة من كورس، أو أي صورة يستخدمها التاجر لعرض منتجه — كلها مقبولة. لو ما كان واضح وش المنتج بالضبط، استنتج منتج رقمي معقول يناسب محتوى الصورة واسم المتجر.
- usable: خلها true دائمًا، إلا لو الصورة غير لائقة (محتوى جنسي، عنف، إساءة) أو فاضية تمامًا بدون أي محتوى؛ وقتها false مع نصوص فاضية و0 للسعر.
...
async function handleStorePreview(req, res) {
  ...
  try {
    ...
    if (!result.usable) {
      return res.status(422).json({ error: "ما نقدر نعرض هذي الصورة. جرّب صورة ثانية لمنتجك (غلاف، تصميم، دعوة...)." });
    }
