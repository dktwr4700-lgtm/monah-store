...
const styles = `
  ...
  .mc-section{margin-top:25px}.mc-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}
  .mc-product{display:flex;flex-direction:column;background:#fff;border:1px solid rgba(17,32,24,.1);border-radius:20px;overflow:hidden;text-decoration:none;color:#112018;transition:transform .18s ease,box-shadow .18s ease}.mc-product:hover{transform:translateY(-3px);box-shadow:0 14px 28px rgba(11,11,12,.08)}
  .mc-product-img{aspect-ratio:1/1.05;background:#EAF0EB;position:relative;display:flex;align-items:center;justify-content:center;overflow:hidden}.mc-product-img img{width:100%;height:100%;object-fit:cover}
  .mc-product-badge{position:absolute;top:9px;inset-inline-start:9px;background:var(--mc-brand);color:#fff;font-size:9.5px;font-weight:800;padding:5px 10px;border-radius:100px}
  .mc-file-card{width:55%;height:72%;background:#FFFDF8;box-shadow:0 10px 18px rgba(11,11,12,.12);border-radius:10px;transform:rotate(-6deg);padding:10px}.mc-file-card b{display:block;width:65%;height:6px;border-radius:8px;background:#C28B3B}.mc-file-card i{display:block;height:4px;background:#EDEAE0;border-radius:7px;margin-top:8px}.mc-file-card i:nth-child(3){width:80%}.mc-file-card i:nth-child(4){width:92%}.mc-file-card em{display:block;height:19px;background:#EAF0EB;border-radius:6px;margin-top:11px}
  .mc-product-body{padding:11px 12px 12px;display:flex;flex-direction:column;gap:5px;flex:1}
  .mc-product-type{font-size:9.5px;color:#9C6D1F;font-weight:800}
  .mc-product-name{font-size:12.5px;font-weight:800;line-height:1.55;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
  .mc-product-desc{font-size:10.5px;color:#4A564C;line-height:1.6;display:-webkit-box;-webkit-line-clamp:1;-webkit-box-orient:vertical;overflow:hidden}
  .mc-price{font-size:13px;font-weight:800;color:var(--mc-brand);margin-top:2px}
  .mc-view{display:block;margin-top:8px;border:1.5px solid var(--mc-brand);color:var(--mc-brand);border-radius:100px;padding:8px;text-align:center;font-size:11px;font-weight:800;transition:background .15s ease,color .15s ease}.mc-product:hover .mc-view{background:var(--mc-brand);color:#fff}
          ...
          {products.length === 0 ? <section className="mc-empty">
            ...
            <section className="mc-section">
              {shownProducts.length ? <div className="mc-grid">{shownProducts.map((product) => <a className="mc-product" href={`#product/${product.id}`} key={product.id}>
                <div className="mc-product-img">
                  {product.images?.[0] ? <img src={product.images[0]} alt={product.name || t.genericProduct} /> : product.previewVideoUrl ? <video src={product.previewVideoUrl} muted preload="metadata" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div className="mc-file-card"><b></b><i></i><i></i><i></i><em></em></div>}
                  {product.featured && <span className="mc-product-badge">{t.featured}</span>}
                </div>
                <div className="mc-product-body">
                  <div className="mc-product-type">{product.category || t.genericProduct}</div>
                  <div className="mc-product-name">{product.name || t.genericProduct}</div>
                  <div className="mc-product-desc">{product.description || t.genericDesc}</div>
                  <span className="mc-price">{Number(product.price || 0).toFixed(2)} {curr}</span>
                  <span className="mc-view">{t.viewProduct}</span>
                </div>
