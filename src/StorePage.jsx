...
const styles = `
  ...
  .st-item-top{ display:flex; justify-content:space-between; align-items:center; }
  .st-item-name{ font-weight:700; color:#16233F; font-size:14px; }
  .st-item-price{ color:#B9832F; font-weight:800; font-size:13px; font-family:'JetBrains Mono',monospace; }
  .st-item-desc{ color:#8A8677; font-size:11.5px; margin-top:6px; line-height:1.6; }
            ...
            {groups[cat].map((p) => (
              <a className="st-item" href={`#product/${p.id}`} key={p.id}>
                <div className="st-item-top">
                  <span className="st-item-name">{p.name}</span>
                  <span className="st-item-price mono">{Number(p.price).toFixed(2)} ر.ع</span>
                </div>
