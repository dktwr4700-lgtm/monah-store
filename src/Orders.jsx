  ...
  return (
    <div className="ord-wrap" dir="rtl" lang="ar">
      ...
      <section className="ord-settings">
        <b>تعليمات التحويل لعملائك</b>
        <p>اكتب بيانات التحويل التي تريد أن تظهر للمشتري داخل الطلب. لا تضع كلمة مرور أو رمز تحقق.</p>
        <textarea value={instructions} onChange={(event) => setInstructions(event.target.value)} placeholder="مثال: حوّل المبلغ إلى الحساب ... ثم ارفع إثبات التحويل هنا." maxLength={800} />
        <button type="button" disabled={savingInstructions} onClick={saveInstructions}>{savingInstructions ? "جاري الحفظ..." : "حفظ تعليمات التحويل"}</button>
        {settingsMessage && <div className={settingsMessage.startsWith("تم") ? "ord-action-note" : "ord-action-error"}>{settingsMessage}</div>}
      </section>
